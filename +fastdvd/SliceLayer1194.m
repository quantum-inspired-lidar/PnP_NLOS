classdef SliceLayer1194 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = SliceLayer1194(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'x166'};
            this.ONNXParams = onnxParams;
        end
        
        function [x166] = predict(this, x)
            if isdlarray(x)
                x = stripdims(x);
            end
            xNumDims = 4;
            onnxParams = this.ONNXParams;
            [x166, x166NumDims] = SliceFcn(x, xNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x166}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1194');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'SliceLayer1194'));
            end
            x166 = dlarray(single(x166), 'SSCB');
            if ~coder.target('MATLAB')
                x166 = extractdata(x166);
            end
        end
        
        function [x166] = forward(this, x)
            if isdlarray(x)
                x = stripdims(x);
            end
            xNumDims = 4;
            onnxParams = this.ONNXParams;
            [x166, x166NumDims] = SliceFcn(x, xNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x166}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1194');
...
function [x166, x166NumDims, state] = SliceFcn(x, xNumDims, params, varargin)
...
[x166, x166NumDims, state] = SliceGraph1191(x, NumDims.x, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x166] = postprocessOutput(x166, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x166, x166NumDims1193, state] = SliceGraph1191(x, xNumDims1192, Vars, NumDims, Training, state)
% Function implementing the graph 'SliceGraph1191'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.x = x;
NumDims.x = xNumDims1192;

% Execute the operators:
% Slice:
[Indices, NumDims.x166] = prepareSliceArgs(Vars.x, [2], [3], [1], [], NumDims.x);
Vars.x166 = subsref(Vars.x, Indices);

% Set graph output arguments from Vars and NumDims:
x166 = Vars.x166;
x166NumDims1193 = NumDims.x166;
...
function [x166] = postprocessOutput(x166, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
x166 = permuteOutputVar(x166, outputDataPerms{1}, 4);
end
