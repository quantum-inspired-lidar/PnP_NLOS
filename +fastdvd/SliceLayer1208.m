classdef SliceLayer1208 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = SliceLayer1208(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'x167'};
            this.ONNXParams = onnxParams;
        end
        
        function [x167] = predict(this, x)
            if isdlarray(x)
                x = stripdims(x);
            end
            xNumDims = 4;
            onnxParams = this.ONNXParams;
            [x167, x167NumDims] = SliceFcn(x, xNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x167}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1208');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'SliceLayer1208'));
            end
            x167 = dlarray(single(x167), 'SSCB');
            if ~coder.target('MATLAB')
                x167 = extractdata(x167);
            end
        end
        
        function [x167] = forward(this, x)
            if isdlarray(x)
                x = stripdims(x);
            end
            xNumDims = 4;
            onnxParams = this.ONNXParams;
            [x167, x167NumDims] = SliceFcn(x, xNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x167}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1208');
...
function [x167, x167NumDims, state] = SliceFcn(x, xNumDims, params, varargin)
...
[x167, x167NumDims, state] = SliceGraph1205(x, NumDims.x, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x167] = postprocessOutput(x167, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x167, x167NumDims1207, state] = SliceGraph1205(x, xNumDims1206, Vars, NumDims, Training, state)
% Function implementing the graph 'SliceGraph1205'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.x = x;
NumDims.x = xNumDims1206;

% Execute the operators:
% Slice:
[Indices, NumDims.x167] = prepareSliceArgs(Vars.x, [3], [4], [1], [], NumDims.x);
Vars.x167 = subsref(Vars.x, Indices);

% Set graph output arguments from Vars and NumDims:
x167 = Vars.x167;
x167NumDims1207 = NumDims.x167;
...
function [x167] = postprocessOutput(x167, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
x167 = permuteOutputVar(x167, outputDataPerms{1}, 4);
end
