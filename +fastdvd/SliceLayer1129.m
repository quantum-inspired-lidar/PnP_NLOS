classdef SliceLayer1129 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = SliceLayer1129(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'x168'};
            this.ONNXParams = onnxParams;
        end
        
        function [x168] = predict(this, x)
            if isdlarray(x)
                x = stripdims(x);
            end
            xNumDims = 4;
            onnxParams = this.ONNXParams;
            [x168, x168NumDims] = SliceFcn(x, xNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x168}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1129');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'SliceLayer1129'));
            end
            x168 = dlarray(single(x168), 'SSCB');
            if ~coder.target('MATLAB')
                x168 = extractdata(x168);
            end
        end
        
        function [x168] = forward(this, x)
            if isdlarray(x)
                x = stripdims(x);
            end
            xNumDims = 4;
            onnxParams = this.ONNXParams;
            [x168, x168NumDims] = SliceFcn(x, xNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x168}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1129');
...
function [x168, x168NumDims, state] = SliceFcn(x, xNumDims, params, varargin)
...
%				  X168:		[Unknown, Unknown, Unknown, Unknown]				Type: FLOAT
...
[x168, x168NumDims, state] = SliceGraph1126(x, NumDims.x, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x168] = postprocessOutput(x168, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x168, x168NumDims1128, state] = SliceGraph1126(x, xNumDims1127, Vars, NumDims, Training, state)
% Function implementing the graph 'SliceGraph1126'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.x = x;
NumDims.x = xNumDims1127;

% Execute the operators:
% Slice:
[Indices, NumDims.x168] = prepareSliceArgs(Vars.x, [4], [5], [1], [], NumDims.x);
Vars.x168 = subsref(Vars.x, Indices);

% Set graph output arguments from Vars and NumDims:
x168 = Vars.x168;
x168NumDims1128 = NumDims.x168;
...
function [x168] = postprocessOutput(x168, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
x168 = permuteOutputVar(x168, outputDataPerms{1}, 4);
end
