classdef SliceLayer1185 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = SliceLayer1185(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'x165'};
            this.ONNXParams = onnxParams;
        end
        
        function [x165] = predict(this, x)
            if isdlarray(x)
                x = stripdims(x);
            end
            xNumDims = 4;
            onnxParams = this.ONNXParams;
            [x165, x165NumDims] = SliceFcn(x, xNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x165}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1185');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'SliceLayer1185'));
            end
            x165 = dlarray(single(x165), 'SSCB');
            if ~coder.target('MATLAB')
                x165 = extractdata(x165);
            end
        end
        
        function [x165] = forward(this, x)
            if isdlarray(x)
                x = stripdims(x);
            end
            xNumDims = 4;
            onnxParams = this.ONNXParams;
            [x165, x165NumDims] = SliceFcn(x, xNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x165}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'SliceLayer1185');
...
function [x165, x165NumDims, state] = SliceFcn(x, xNumDims, params, varargin)
...
[x165, x165NumDims, state] = SliceGraph1182(x, NumDims.x, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x165] = postprocessOutput(x165, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x165, x165NumDims1184, state] = SliceGraph1182(x, xNumDims1183, Vars, NumDims, Training, state)
% Function implementing the graph 'SliceGraph1182'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.x = x;
NumDims.x = xNumDims1183;

% Execute the operators:
% Slice:
[Indices, NumDims.x165] = prepareSliceArgs(Vars.x, [1], [2], [1], [], NumDims.x);
Vars.x165 = subsref(Vars.x, Indices);

% Set graph output arguments from Vars and NumDims:
x165 = Vars.x165;
x165NumDims1184 = NumDims.x165;
...
function [x165] = postprocessOutput(x165, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
x165 = permuteOutputVar(x165, outputDataPerms{1}, 4);
end
